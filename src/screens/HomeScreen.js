...
// Skeleton Component
const SkeletonBlock = ({ width: w = "100%", height: h = 16, borderRadius: br = 8, style = {} }) => {
  ...
  return (
    <Animated.View style={[{ width: w, height: h, borderRadius: br, backgroundColor: "rgba(255,255,255,0.15)", opacity }, style]} />
  );
};

const OrderSkeleton = () => (
  <View style={styles.orderCard}>
    <View style={[styles.orderAccent, { backgroundColor: "rgba(255,255,255,0.1)" }]} />
    <View style={{ flex: 1, padding: 14, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <SkeletonBlock width={70} height={24} borderRadius={20} />
    </View>
  </View>
);
  ...
  return (
    <View style={styles.container}>
      ...
      <ImageBackground source={{ uri: HERO_IMAGE }} style={StyleSheet.absoluteFill} resizeMode="cover">
        <LinearGradient colors={["rgba(5,8,16,0.92)", "rgba(13,17,23,0.88)", "rgba(17,24,39,0.94)"]} style={StyleSheet.absoluteFill} />
      </ImageBackground>
      ...
      <View style={styles.gridContainer}>
        {[...Array(8)].map((_, i) => <View key={i} style={[styles.gridLine, { left: (width / 8) * i }]} />)}
        {[...Array(15)].map((_, i) => <View key={i} style={[styles.gridLineH, { top: (height / 15) * i }]} />)}
      </View>
      ...
      >
        ...
        <Animated.View style={[styles.heroSection, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          <View style={styles.heroCard}>
            <LinearGradient colors={["rgba(231,76,60,0.15)", "rgba(192,57,43,0.05)", "rgba(0,0,0,0)"]} style={styles.heroCardGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} />
            <View style={styles.cornerTL} />
            <View style={styles.cornerTR} />
            <View style={styles.cornerBL} />
            <View style={styles.cornerBR} />
            <Text style={styles.heroTag}>◈ NIGERIA'S PREMIER LOGISTICS</Text>
            <Text style={styles.heroTitle}>Delivering Your{"\n"}<Text style={styles.heroTitleAccent}>Dreams</Text>{"\n"}Across Nigeria</Text>
            <Text style={styles.heroSubtitle}>Real-time tracking • Instant notifications • 99% success rate</Text>
        ...
        <View style={styles.orderStatsRow}>
          {[
            { label: "Active", value: loading ? "—" : activeOrders.length, color: "#f39c12", icon: "🚚" },
            { label: "Delivered", value: loading ? "—" : deliveredOrders.length, color: "#27ae60", icon: "✅" },
            { label: "Total", value: loading ? "—" : orders.length, color: "#3498db", icon: "📦" }
          ].map((stat, i) => (
            <View key={i} style={[styles.orderStatCard, { borderColor: stat.color + "40" }]}>
              <LinearGradient colors={[stat.color + "20", "transparent"]} style={styles.orderStatGradient}>
                ...
                {loading ? <SkeletonBlock width={30} height={22} style={{ alignSelf: "center", marginVertical: 2 }} /> : (
        ...
        <View style={[styles.sectionHeader, { marginTop: 20 }]}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {orders.length > 0 && (
              <TouchableOpacity onPress={() => navigation.navigate("BookDelivery")}>
                <Text style={{ color: "#e74c3c", fontSize: 12, fontWeight: "700" }}>+ New →</Text>
              </TouchableOpacity>
            )}
          </View>
        </View>
        ...
        {loading ? (
          ...
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptySubtitle}>Book your first delivery to get started!</Text>
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {orders.map((order, i) => (
              <TouchableOpacity key={i} onPress={() => navigation.navigate("Track", { trackingNumber: order.trackingNumber })}
                style={[styles.orderCard, { borderColor: getStatusColor(order.status) + "40" }]}>
                <View style={[styles.orderAccent, { backgroundColor: getStatusColor(order.status) }]} />
                <View style={styles.orderContent}>
                  <View style={styles.orderLeft}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 4 }}>
                      <Text style={{ fontSize: 14 }}>{getStatusIcon(order.status)}</Text>
                    ...
                    <Text style={styles.orderDate}>📅 {new Date(order.createdAt).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  orderStatCard: { flex: 1, borderRadius: 14, borderWidth: 1, overflow: "hidden" },
  ...
  emptySubtitle: { fontSize: 14, color: "rgba(255,255,255,0.3)", marginBottom: 20, textAlign: "center" },
  ...
  orderLeft: { flex: 1 },
  orderTracking: { fontSize: 13, fontWeight: "800", color: "#3498db" },
  orderTo: { fontSize: 12, color: "rgba(255,255,255,0.4)", marginBottom: 3 },
  orderDate: { fontSize: 11, color: "rgba(255,255,255,0.25)" },
  orderBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, borderWidth: 1, marginLeft: 8 },
